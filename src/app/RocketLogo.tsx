 ...
 * Just the hop-rocket body, exhaust fins, and lightning bolt.
 */
function RocketOnly({ size = 165 }: { size?: number }) {
  const fill = "#044350";
  return (
    <svg
      width={size * 0.72}
      height={size}
      viewBox="0 0 31.475842 43.711811"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g transform="translate(-113.77083,-128.32291)">
        <g transform="matrix(0.26458333,0,0,0.26458333,24.601966,128.05833)">
          {/* Main rocket body */}
          <path
...
export function MainRocketLogo() {
  ...
  const fire = useCallback(() => {
    ...
    const rot = shouldRotate ? (Math.random() - 0.5) * 30 : 0;
    setDuration(dur);
    setRotation(rot);
    setFiring(true);
    setTimeout(() => {
  ...
  return (
    <Flex alignItems="center" gap="6" position="relative" overflow="visible">
      ...
      >
        ...
        {/* Flame particles — positioned at the exhaust */}
        <Box
          position="absolute"
          bottom="-62px"
          left="41%"
          transform="translateX(-50%)"
          className={firing ? "rocket-flames-active" : "rocket-flames"}
        >
